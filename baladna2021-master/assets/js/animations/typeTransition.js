...
// .type_line CSS opacity value (CSS variable)
...
export class TypeTransition {
    ...
    constructor(DOM_el) {
        this.DOM.el = DOM_el;
        // lines of text
        this.DOM.lines = [...document.querySelectorAll('.type_line')];
    }
    ...
    out() {
        return gsap.timeline({ paused: true })
            .set(this.DOM.el, { scale: 2.7, rotate: -90 })
            .set(this.DOM.lines, { x: '-200%', opacity: 0 })
            .to(this.DOM.el, {
                duration: 1.4,
                ease: 'power2.inOut',
                scale: 1,
